...
7. **Structured Output** (~28 lines) — JSON schema dict path (no extra deps) and Pydantic model path with code examples. Note that `response_model` auto-sets `output_format="json"`.
